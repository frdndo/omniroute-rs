...
pub fn init_database(path: String) -> napi::Result<()> {
    ...
    DB.set(db).map_err(|_| napi::Error::from_reason("DB already initialized"))?;
...
pub fn get_provider_connections() -> napi::Result<Vec<serde_json::Value>> {
    let db = DB.get().ok_or_else(|| napi::Error::from_reason("DB not initialized"))?;
    let conn = db.conn.lock().map_err(|e| napi::Error::from_reason(e.to_string()))?;
    let connections = omniroute_db::repos::provider_connection_repo::get_all(&conn)
        .map_err(|e| napi::Error::from_reason(e.to_string()))?;
    Ok(connections.into_iter().map(|c| serde_json::to_value(c).unwrap()).collect())
}
...
pub fn list_providers() -> napi::Result<Vec<serde_json::Value>> {
    ...
    Ok(providers.into_iter().map(|p| serde_json::to_value(p).unwrap()).collect())
}
